...
func (ibmq *IBMQ) ConnectToQueueManager() bool {
	...
	goQMgrName := config.CfgObj.IBMQConfigDetails.MQManager

	channel := config.CfgObj.IBMQConfigDetails.MQChannel
	connName := config.CfgObj.IBMQConfigDetails.MQConnection
	// user := config.CfgObj.IBMQConfigDetails.MQManager
	// password := config.CfgObj.IBMQConfigDetails.MQManager

	cno := ibmmq.NewMQCNO()
	sco := ibmmq.NewMQSCO()
	cd := ibmmq.NewMQCD()

	cd.SSLCipherSpec = config.CfgObj.IBMQConfigDetails.MQTLSCipher
	sco.KeyRepository = config.CfgObj.IBMQConfigDetails.MQRepoLoc
	...
	if err != nil {
		...
		fmt.Println("Connected to queue manager {" + goQMgrName + "}")

		if !ibmq.ConnectToQueue(config.CfgObj.IBMQConfigDetails.MQQueue) {
			return false
		}
	}
...
func (ibmq *IBMQ) ConnectToQueue(queue string) bool {
	...
	openOptions := ibmmq.MQOO_INPUT_SHARED | ibmmq.MQOO_OUTPUT
	...
	if err != nil {
		...
		fmt.Println("Connected to Queue {" + config.CfgObj.IBMQConfigDetails.MQQueue + "}")
	}
...
func (ibmq *IBMQ) PutMessageIntoQueue(message []byte) error {
	...
	return ibmq.iBMQObject.Put(putmqmd, gopmo, message)
}
...
func (ibmq *IBMQ) GetMessageFromQueue() (string, error) {
	...
	datalen, err := ibmq.iBMQObject.Get(getmqmd, gmo, buffer)

	if err != nil {
		log.Println("error fetching msgs from queue: ", err)
		return "", err
	} else {
		fmt.Println("fetched form queue")
	}
